...
with tab_noten:
    ...
    for i, e in enumerate(st.session_state.noten_eintraege):
        ...
        note = c1.number_input("", 1.0, 6.0, float(e["note"]), 0.1, format="%.1f", key=f"n_val_{i}")
        gewicht_sel = c2.selectbox("", [0.5, 1.0, 2.0, 3.0], index=[0.5,1,2,3].index(e["gewicht"]) if e["gewicht"] in [0.5,1,2,3] else 1,
                                   format_func=lambda x: f"×{x:.1f}" if x != 1 else "normal", key=f"n_gew_{i}")
        ist_2fach = c3.checkbox("", value=(e["gewicht"] >= 2.0), key=f"n_2f_{i}")
        gewicht = 2.0 if ist_2fach else gewicht_sel

        if c4.button("×", key=f"del_n_{i}"):
    ...
    col_a.button("➕ Note hinzufügen", use_container_width=True,
                 on_click=lambda: st.session_state.noten_eintraege.append({"note": 2.0, "gewicht": 1.0}))
    col_b.button("↺ Liste löschen", use_container_width=True,
                 on_click=lambda: setattr(st.session_state, "noten_eintraege", [{"note": 2.0, "gewicht": 1.0}]))

    # Export CSV – Noten
    if st.session_state.noten_eintraege:
        df_export = pd.DataFrame([
            {"Note": round(e["note"], 1), "Gewichtung": e["gewicht"], "Beitrag": round(e["note"] * e["gewicht"], 2)}
            for e in st.session_state.noten_eintraege
        ])
        csv = df_export.to_csv(index=False).encode('utf-8')
        col_c.download_button("CSV exportieren", csv, "noten_schnitt.csv", "text/csv", use_container_width=True)
...
with tab_punkte:
    ...
    for i, e in enumerate(st.session_state.punkte_eintraege):
        ...
        punkte = c1.number_input("", 0, 15, int(e["punkte"]), 1, key=f"p_val_{i}")
        note = punkte_zu_note(punkte)
        c2.markdown(f"**{note:.2f}**" if note <= 4.0 else f"{note:.2f}")

        gewicht_sel = c3.selectbox("", [0.5, 1.0, 2.0, 3.0], index=[0.5,1,2,3].index(e["gewicht"]) if e["gewicht"] in [0.5,1,2,3] else 1,
                                   format_func=lambda x: f"×{x:.1f}" if x != 1 else "normal", key=f"p_gew_{i}")
        ist_2fach = c4.checkbox("", value=(e["gewicht"] >= 2.0), key=f"p_2f_{i}")
        gewicht = 2.0 if ist_2fach else gewicht_sel

        if c5.button("×", key=f"del_p_{i}"):
    ...
    col_a, col_b, col_c = st.columns([2, 2, 1])
    col_a.button("➕ Punkte hinzufügen", use_container_width=True,
    ...
    if st.session_state.punkte_eintraege:
        df_export = pd.DataFrame([
            {"Punkte": e["punkte"], "Note": round(e["note"], 2), "Gewichtung": e["gewicht"], "Beitrag": round(e["note"] * e["gewicht"], 2)}
            for e in st.session_state.punkte_eintraege
        ])
        csv = df_export.to_csv(index=False).encode('utf-8')
        col_c.download_button("CSV exportieren", csv, "punkte_schnitt.csv", "text/csv", use_container_width=True)
    ...
    with st.expander("Vollständige Umrechnungstabelle (Punkte → Note)", expanded=False):
        ...
        for p in range(15, -1, -1):
            n = punkte_zu_note(p)
            table_data.append({"Punkte": p, "Note": f"{n:.2f}"})

        df_table = pd.DataFrame(table_data)
        st.dataframe(df_table, hide_index=True, use_container_width=True)

st.caption("• (17 – Punkte) ÷ 3 • Gewichtung 0,5–3,0× • CSV-Export enthält Beiträge • Stand 2026")
